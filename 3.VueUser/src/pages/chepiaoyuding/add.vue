...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="车票编号" prop="chepiaobianhao">
            <el-input v-model="ruleForm.chepiaobianhao" 
                placeholder="车票编号" clearable :disabled=" false  ||ro.chepiaobianhao"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="座位类型" prop="zuoweileixing">
            <el-input v-model="ruleForm.zuoweileixing" 
                placeholder="座位类型" clearable :disabled=" false  ||ro.zuoweileixing"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="车辆照片" v-if="type!='cross' || (type=='cross' && !ro.cheliangzhaopian)" prop="cheliangzhaopian">
            <file-upload
            tip="点击上传车辆照片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' class="upload" v-else label="车辆照片" prop="cheliangzhaopian">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="车票数量" prop="chepiaoshuliang">
            <el-input v-model.number="ruleForm.chepiaoshuliang" 
                placeholder="车票数量" clearable :disabled=" false  ||ro.chepiaoshuliang"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`chepiaoyuding/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('chepiaoyuding/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`chepiaoyuding/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
