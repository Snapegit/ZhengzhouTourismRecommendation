...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="车票编号" prop="chepiaobianhao">
              <el-input v-model="ruleForm.chepiaobianhao" placeholder="车票编号" disabled></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="车站名称" prop="chezhanmingcheng">
            <el-input v-model="ruleForm.chezhanmingcheng" 
                placeholder="车站名称" clearable :disabled=" false  ||ro.chezhanmingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="车辆名称" prop="cheliangmingcheng">
            <el-input v-model="ruleForm.cheliangmingcheng" 
                placeholder="车辆名称" clearable :disabled=" false  ||ro.cheliangmingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="座位类型" prop="zuoweileixing">
            <el-select v-model="ruleForm.zuoweileixing" placeholder="请选择座位类型" :disabled=" false  ||ro.zuoweileixing" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="出发地" prop="chufadi">
            <el-input v-model="ruleForm.chufadi" 
                placeholder="出发地" clearable :disabled=" false  ||ro.chufadi"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="目的地" prop="mudedi">
            <el-input v-model="ruleForm.mudedi" 
                placeholder="目的地" clearable :disabled=" false  ||ro.mudedi"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="出发时间" prop="chufashijian">
              <el-date-picker
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="车票价格" prop="chepiaojiage">
			<el-input-number v-model="ruleForm.chepiaojiage" placeholder="车票价格" :readonly="ro.chepiaojiage"></el-input-number>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="班列介绍" prop="banliejieshao">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="班列介绍"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`chepiaoxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('chepiaoxinxi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`chepiaoxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
