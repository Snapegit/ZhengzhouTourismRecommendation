...
<div>
	...
	<div class="list-preview" :style='{"padding":"0 7%","margin":"0px auto","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"100%"}'>
		...
		<div class="category-3" :style='{"border":"0px solid #eee","padding":"26px 20px 0","margin":"20px auto 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}'>
			...
			<div class="item" :class="swiperIndex == index ? 'active' : ''" v-for="(item, index) in fenlei" :key="index" @click="getList(1, item[feileiColumn], 'btn' + index)" :ref="'btn' + index" plain>
				<img v-if="item.image" :style='{"width":"50px","margin":"5px 5px 0 0","objectFit":"cover","display":"block","height":"50px"}' :src="baseUrl + (item.image?item.image.split(',')[0]:'')">
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 20px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 1px","display":"flex","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"100px","padding":"0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>美食名称：</div>
        <el-input v-model="formSearch.meishimingcheng" placeholder="美食名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"100px","padding":"0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>美食地点：</div>
        <el-input v-model="formSearch.meishididian" placeholder="美食地点" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"100px","padding":"0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>美食口味：</div>
        <el-input v-model="formSearch.meishikouwei" placeholder="美食口味" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('meishixinxi','新增')" :style='{"cursor":"pointer","border":"1px solid #ddd","padding":"0px 15px","margin":"0px 10px 0 0","outline":"none","color":"#333","borderRadius":"0px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/meishixinxiAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"width":"100%","padding":"0 0px","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"0px solid #f6f6f6","width":"100%","padding":"0px","margin":"0 0px 0 0","overflow":"hidden","height":"240px"}' class="img-box">
				<img @click.stop="imgPreView(item.fengmian)" :style='{"width":"100%","objectFit":"cover","display":"block","clipPath":"polygon(100% 88%, 88% 100%, 0% 100%, 0% 0%, 100% 0%)","height":"100%"}' v-if="item.fengmian && item.fengmian.substr(0,4)=='http'" :src="item.fengmian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.fengmian?item.fengmian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","clipPath":"polygon(100% 88%, 88% 100%, 0% 100%, 0% 0%, 100% 0%)","height":"100%"}' v-else :src="baseUrl + (item.fengmian?item.fengmian.split(',')[0]:'')" class="image" />
			</div>
		    <div :style='{"width":"100%","padding":"0 0px","overflow":"hidden","alignItems":"center","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"55%","padding":"0 0px 0 0","flex":"1"}'>
				<div class="title">{{item.meishimingcheng}}</div>
				<div v-if="item.price" class="price"><span :style='{"fontSize":"12px"}'>￥</span>{{item.price}}</div>
		        <div :style='{"padding":"2px 0px","borderColor":"#f0f0f0","borderStyle":"dashed","borderWidth":"0 0 1px 0"}' class="time">
		          <span class="icon iconfont icon-shijian21"></span>
		          <span class="text">{{item.addtime}}</span>
		        </div>
		        <div :style='{"width":"33%","padding":"2px 10px 0 0","borderColor":"#eee","borderStyle":"dashed","borderWidth":"0 0 1px 0","display":"inline-block"}' class="collect">
		          <span class="icon iconfont icon-shoucang10"></span>
		          <span class="text">{{item.storeupnum}}</span>
		        </div>
		        <div :style='{"width":"34%","padding":"2px 10px 0 0","borderColor":"#eee","borderStyle":"dashed","borderWidth":"0 0 1px 0","display":"inline-block"}' class="view">
		          <span class="icon iconfont icon-chakan9"></span>
		          <span class="text">{{item.clicknum}}</span>
		        </div>
		      </div>
		      <div class="desc">{{item.meishitese}}</div>
		    </div>
		  </div>
		</div>
	</div>
  ...
  export default {
    ...
    created() {
		...
		this.$http.get('option/meishifenlei/meishifenlei').then(res => {
			if (res.data.code == 0) {
				...
				this.selectOptionsList.push({name:'美食分类',list:this.meishifenleiOptions,tableName: 'meishifenlei',check: -1})
			}
		});
	    this.meishipingfenOptions = '好评,差评'.split(',');
		this.selectOptionsList.push({name:'美食评分',list:this.meishipingfenOptions,tableName: 'meishipingfen',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getFenlei() {
		...
		this.feileiColumn = 'meishifenlei'
      },
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.meishimingcheng != '') searchWhere.meishimingcheng = '%' + this.formSearch.meishimingcheng + '%';
        if (this.formSearch.meishifenlei != '') searchWhere.meishifenlei = this.formSearch.meishifenlei;
        if (this.formSearch.meishididian != '') searchWhere.meishididian = '%' + this.formSearch.meishididian + '%';
        if (this.formSearch.meishipingfen != '') searchWhere.meishipingfen = this.formSearch.meishipingfen;
        if (this.formSearch.meishikouwei != '') searchWhere.meishikouwei = '%' + this.formSearch.meishikouwei + '%';
        if (this.curFenlei != '全部') searchWhere.meishifenlei = this.curFenlei;
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`meishixinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/meishixinxiDetail', query: params});
      },
